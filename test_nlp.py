...
def main(model_file):
    ...
    idf = IDF(inputCol='tf', outputCol="features_1", minDocFreq=5)
    vec_assembler = VectorAssembler(inputCols=["features_1"]\
     , outputCol="features")

    #label_stringIdx = StringIndexer(inputCol = "label", outputCol = "target",handleInvalid="error")
    pipeline = Pipeline(stages=[tokenizer, hashtf, idf,vec_assembler])
